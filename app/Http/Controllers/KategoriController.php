...
class KategoriController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // variabel untuk menampung semua data tabel kategori atau search dari view mysql
        $query = DB::table('view_kategori');
        ...
        if ($search) {
            $query->where(function ($query) use ($search) {
                $query->where('deskripsi', 'like', '%' . $search . '%')
                      ...
                      ->orWhere(DB::raw('ketKategorik(kategori) COLLATE utf8mb4_unicode_ci'), 'like', '%' . $search . '%');
            });
        }

        $rsetKategori = $query->orderBy('id', 'desc')->paginate(6);
